  ...
  try {
    ...
    for (let key in userInformation) {
      if (userInformation.hasOwnProperty(key)) {
        Cookies.set(key, userInformation[key], { expires: 1 / 24 });
      }
    }
    ...
    const role = res.data.metadata.user.user_role_name;
    if (role === 'customer') {
    ...
    if (error.status === 403) {
      toast.error('Tài khoản chưa được kích hoạt hoặc đã bị vô hiệu!');
    }
    return false;
  }
};
...
export const getAllUserRole = async () => {
  try {
    const res = await axios.get(`${REACT_APP_BASE_URL}user-role`);
...
export const activateUser = async (accessToken, userID, activateID, axiosJWT) => {
  try {
    const res = await axiosJWT.patch(`${REACT_APP_BASE_URL}user/${activateID}/${userID}`, {
      headers: {
        'Content-Type': 'application/json',
        authorization: `${accessToken}`,
        user: `${userID}`,
...
export const getInvoicesByAppointments = async (appointmentIds, dispatch) => {
  try {
    const idsParam = appointmentIds.join(','); // ["a","b","c"] → "a,b,c"

    const res = await axios.get(`${REACT_APP_BASE_URL}invoice/by-appointments?ids=${idsParam}`, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const exportInvoice = async (invoiceId) => {
  try {
    const response = await axios.get(`${REACT_APP_BASE_URL}invoice/export/${invoiceId}`, {
...
export const deleteInvoice = async (accessToken, invoiceId) => {
  try {
    const res = await axios.delete(`${REACT_APP_BASE_URL}invoice/${invoiceId}`, {
      headers: {
        authorization: `${accessToken}`,
      },
    });
...
export const updateInvoiceStatus = async (accessToken, invoiceId, status, dispatch) => {
  dispatch(updateInvoiceStart());
  try {
    const res = await axios.put(
      `${REACT_APP_BASE_URL}invoice/${invoiceId}/status`,
      { status },
      {
        headers: {
          'Content-Type': 'application/json',
...
export const contactViaEmail = async (data) => {
  try {
    await axios.post(`${REACT_APP_BASE_URL}mail/contact`, data, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const contactAboutRedemptionViaEmail = async (data) => {
  try {
    const res = await axios.post(`${REACT_APP_BASE_URL}mail/redemption`, data, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const getImagesByCollection = async (collectionId) => {
  try {
    const res = await axios.get(`${REACT_APP_BASE_URL}footer/images/collection/${collectionId}`, {
...
export const createFooterCollection = async (data) => {
  try {
    const res = await axios.post(`${REACT_APP_BASE_URL}footer/collection`, data, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const createLookbookCollection = async (data) => {
  try {
    const res = await axios.post(`${REACT_APP_BASE_URL}lookbook/collection`, data, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const setLookbookImageActive = async (id, active) => {
  try {
    const res = await axios.put(`${REACT_APP_BASE_URL}lookbook/image/${id}/active`, { active });
    return res.data;
  } catch (error) {
